...
(async function () {
  ...
  function renderConfig() {
    ...
    document.getElementById("panel-config").innerHTML = `
      <div class="alert alert-info" style="font-size:.88rem">These levers control platform revenue. In production they live in the Go backend config and apply to new transactions. Editing here updates the live demo session.</div>
  ...
  function renderData() {
    document.getElementById("panel-data").innerHTML = `
      <div class="card" style="max-width:560px">
        ...
        <p class="muted" style="font-size:.92rem">All data is stored in your browser via <code>localStorage</code>. This makes the static site fully functional without a server. When the Go + MySQL backend is connected, flip <code>API.state.useBackend = true</code> in <code>js/api.js</code> and this data layer talks to real REST endpoints instead.</p>
