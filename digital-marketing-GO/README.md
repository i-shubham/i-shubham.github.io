...
in the browser via `localStorage`) and is structured to drop in a **Go +
...
1. Build the Go service (handlers mirror the methods in `api.js`).
2. In `js/api.js` set:
  ```js
...
- **Go 1.22+** with **Chi** router (lightweight, idiomatic, stdlib-compatible)
- **sqlc** (type-safe SQL → Go code generation) or **GORM** (ORM)
- **MySQL 8** (or PlanetScale/RDS) via `go-sql-driver/mysql`
- **JWT** auth via `golang-jwt/jwt` (`Authorization: Bearer …` — `api.js` already sends it)
...
- **Built for concurrency:** goroutines handle millions of connections with minimal memory overhead.
- **Fast compilation:** entire backend compiles in seconds — rapid iteration.
- **Single static binary:** deploy a ~15 MB image with zero dependencies.
- **Battle-tested at scale:** Discord, Uber, Cloudflare, Twitch all run Go for high-throughput services.
- **Simple to hire for:** approachable syntax, large talent pool, excellent standard library.
